...
if option =='API認証とデータ取得':
    ...
    if uploaded_file is not None:
         ...
         st.write('### パラメータの設定')
         num_serch_tweet = st.slider('検索件数', 10, 1000, 50)
         if st.button('データ取得'):
            message = st.empty()
            message.write('取得中です。')
            ...
            data = []
            for tweet in tweepy.Paginator(client.get_users_tweets, user_id, exclude = excludes, tweet_fields =tweet_fields).flatten(limit=num_serch_tweet):
